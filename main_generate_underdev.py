...
if __name__ == "__main__":
    ...
    args.add_argument("--seed", default= 42, help=" Generation seed", type= str)
    ...
    if args.dataset_name == "com2sense":
        if args.mode == "normal":
            ...
            args.out_filename = f"./data/{args.dataset_name}/dev_G_normal.pkl"
        elif args.mode == "tilde":
            args.data_filename = f"./data/{args.dataset_name}/bothdev.Q.json"
            args.out_filename = f"./data/{args.dataset_name}/dev_G_tilde.pkl"
    ...
    if args.end > args.start:
        args.out_filename = args.out_filename + f"_{args.start}_{args.end}"
    ...
    print(f"The config for this experiment : {config}")

    if args.end > args.start : 
        for i in tqdm(range(len(samples), args.start, args.end + 1), total=len(samples)):
            sample = samples[i]
            if args.dataset_name == "com2sense":
                if args.mode == "normal":
